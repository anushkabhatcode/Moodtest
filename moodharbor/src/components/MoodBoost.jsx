...
const MoodBoost = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        const response = await fetch('https://moodtest-4.onrender.com/recommend', {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await fetch('https://moodtest-4.onrender.com/save', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
  ...
  return (
    ...
    <main> 
      ...
      <Card
        ...
        {/* {user && mood && desiredMood && `Hi ${user}! Today, you are ${mood}. 
        Check out recommendations to make you ${desiredMood}`}  */}
      </Card>
    ...
    <div className="card-sections-dash">
      ...
      <CardLong
        icon={<FaDailymotion />}
        itemName="Tools"
        description={quote && `${quote} - ${author}`}
      />
        {/* <CardLong 

          icon={<FaSprayCan />}
          itemName="Tools"
          //if category of recommendation is Quotes, display it here
          description={
